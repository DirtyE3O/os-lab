...
public:
    ...
    void renderLine(int j, color* rendered_image, const hittable&world) {
        for (int i = 0; i < image_width; i++) {
            ...
            rendered_image[pixel_index] = pixel_samples_scale * pixel_color;
        }
    }

    void render_parallel(const hittable&world, int num_processes) {
        initialize();
        ...
        for (pid_t pid: child_pids) {
            ...
            if (WIFEXITED(status)) {
                std::clog << "Child " << pid << " finished with status "
        ...
        for (int j = 0; j < image_height; j++) {
            for (int i = 0; i < image_width; i++) {
                int pixel_index = j * image_width + i;
                write_color(std::cout, rendered_image[pixel_index]);
            }
        }
